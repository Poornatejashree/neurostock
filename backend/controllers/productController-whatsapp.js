...
const { sendLowStockWhatsApp, sendOutOfStockWhatsApp } = require("../services/whatsappService");
  ...
  try {
    ...
    if (old && updated && old.quantity !== updated.quantity) {
      ...
      if (updated.quantity === 0) {
        ...
        await Promise.all([
          sendOutOfStockAlert(updated).catch(console.error),
          sendOutOfStockWhatsApp(updated).catch(console.error),
        ]);
        ...
        // Send both email + WhatsApp
        await Promise.all([
          sendLowStockAlert(updated).catch(console.error),
          sendLowStockWhatsApp(updated).catch(console.error),
        ]);
      } else {
